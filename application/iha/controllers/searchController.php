...
class iha_searchController extends controller
{
	...
	public function appointmentAction(){
       ...
       public function search_card_statusAction()
       {
           ...
           $search['start_time'] = date("Y-m-d H:i:s",$start_time);
           $search['end_time']=date("Y-m-d H:i:s",$end_time);
           $this->view->start_time = date("Y-m-d H:i:s",$start_time);
           $this->view->end_time = date("Y-m-d H:i:s",$end_time);
           if(!empty($identity_number))
           {
               $where_str = "  and  iha_card_status.identity_number='$identity_number' ";
           }
           else
           {
               $where_str = "";
           ...
           $iha_card_status_list->query("select * from (select A.*,ROWNUM as RN from (select * from (select iha_card_status.identity_number as c,max(iha_card_status.created) as d from iha_card_status where iha_card_status.identity_number is not null and iha_card_status.identity_number in (select individual_core.identity_number from individual_core) and  iha_card_status.created>=$start_time and  iha_card_status.created<=$end_time $where_str group by iha_card_status.identity_number) order by d DESC ) A where ROWNUM <= $endnum) where RN > $startnum");
           $row = array();
           $i = 0;
           while($iha_card_status_list->fetch())
           {
               //取得病人姓名
               $individual_core = new Tindividual_core();
               $individual_core->whereAdd("identity_number='$iha_card_status_list->c'");
               $individual_core->find(true);
               $row[$i]['name']    = $individual_core->name;
               //$sex = array(0=>'未知的性别', 1=>'男', 2=>'女', 9=>'未说明的性别');
              // $row[$i]['sex'] = $sex[$individual_core->sex];
               $row[$i]['identity_number']    = base64_encode($iha_card_status_list->c);
               $i++;
           }
           $this->view->row = $row;
