...
*@create：2013-10-18
*/
class iha_repeatnameController extends controller
{
	...
	public function indexAction()
	{
        ...
        $user_name = trim($this->_request->getParam('user_name'));//身份证号
        $token=$this->_request->getParam("token");//所属地区标致
        if(!empty($user_name))
        {
        	$where_1.="  and name='$user_name' ";
        	$where_2.=" where name='$user_name'";
        }
        if($token==1){
        	//管辖范围
        	$sql2="SELECT  count(*) as num  from (select *  from (select name, count(name) as nums  from individual_core where region_path like '$current_region_path%'".$where_1." group by name order by nums desc )  where nums > 1)";//查询总记录数
        }else{
        	//所有数据
        	$sql2="SELECT  count(*) as num  from (select *  from (select name, count(name) as nums  from individual_core".$where_2." group by name order by nums desc )  where nums > 1)";//查询总记录数
        }
		...
		$search=array('name'=>$user_name);
		$pageofnumber=20;
		$links=new SubPages($pageofnumber,$total,$pageCurrent,__goodsListRowOfPage,__BASEPATH.'iha/repeatname/index/token/'.$token.'/page/',2,$search);
		...
		$endnum=$startnum+$pageofnumber;
		 if($token==1){
        	//管辖范围
			$sql="SELECT * FROM (select ROWNUM as r,name,nums  from (select name, count(name) as nums  from individual_core  where region_path like '$current_region_path%'".$where_1." group by name order by nums desc ) where nums > 1 )WHERE r>=".$startnum." AND r<".$endnum; //oracle中
		 }else{
			$sql="SELECT * FROM (select ROWNUM as r,name,nums  from (select name, count(name) as nums  from individual_core".$where_2." group by name order by nums desc ) where nums > 1 )WHERE r>=".$startnum." AND r<".$endnum; //oracle中
		 }
...
//			$individual[$i]['identity_number']=$individual_core->identity_number;
	...
	public function listAction()
	{
		...
		if($user_name!="")
		{
			if($token!="")
			{
				//查询本地区内的重档数2013-10-18
				$individual_core=new Tindividual_core();
				$individual_core->whereAdd("name='$user_name'");
				...
				$pageofnumber=20;
				$links=new SubPages($pageofnumber,$nums,$pageCurrent,__goodsListRowOfPage,__BASEPATH.'iha/repeatname/list/name/'.$user_name.'/token/'.$token.'/page/',2,$search);
				$pageCurrent=$links->check_page($pageCurrent);
			...
			}else 
			{
				...
				//new subpages(每页显示条数，总条数，当前页数，每次显示页数索引，URL地址，样式，URL参数数组);
				$pageofnumber=20;
				$links=new SubPages($pageofnumber,$nums,$pageCurrent,__goodsListRowOfPage,__BASEPATH.'iha/repeatname/list/name/'.$user_name.'/page/',2,$search);
				$pageCurrent=$links->check_page($pageCurrent);
				$startnum=$pageofnumber*($pageCurrent-1);
				$individual_core->limit($startnum,$pageofnumber);
		...
		else 
		{
			...
			$individual_core2=new Tindividual_core();
			$sql2="select count(*) as num from individual_core where region_path like '$current_region_path%' and name in (select name from (select name,count(name) as nums from INDIVIDUAL_CORE GROUP BY name ORDER BY nums DESC) where nums>1) ORDER BY name desc";
			$individual_core2->query($sql2);
			$individual_core2->fetch();
			$total=$individual_core2->num;
//			echo $total;
//			exit();
			...
			$search=array();
			$pageofnumber=20;
			$links=new SubPages($pageofnumber,$total,$pageCurrent,__goodsListRowOfPage,__BASEPATH.'iha/repeatname/list/page/',2,$search);
			$pageCurrent=$links->check_page($pageCurrent);
			$startnum=$pageofnumber*($pageCurrent-1)+1;//使用rownum时，查询返回值从第1行开始
			$endnum=$startnum+$pageofnumber;
...
//			$sql="select * from (select individual_core.*,ROWNUM as r from individual_core where region_path like '$current_region_path%' and identity_number in (select identity_number from (select identity_number,count(identity_number) as nums from INDIVIDUAL_CORE GROUP BY IDENTITY_NUMBER ORDER BY nums DESC) where nums>1) ORDER BY IDENTITY_NUMBER desc) where r>".$startnum." AND r<".$endnum;//运行但不能分页
			$sql="select * from (select * from (select individual_core.*,ROWNUM as r from individual_core where region_path like '$current_region_path%' and name in (select name from (select name,count(name) as nums from INDIVIDUAL_CORE GROUP BY name ORDER BY nums DESC) where nums>1) ORDER BY name desc) where r<=".$endnum.") where r>".$startnum;//运行能分页 使用rownum别名 只要大于1就可以
//			echo $sql;
			$individual_core->query($sql);
			$individual=array();
			$i=0;
