...
class Oci8{
	...
	public function set_modle_path($path){
		if($dir=$this->check_dir($path)){
			...
			throw new Exception('请设置好model生成文件的路径!');
		}
	...
	public function set_view_path($path){
		if($dir=$this->check_dir($path)){
			...
			throw new Exception('请设置好view生成文件的路径!');
		}
	}
	/**
	 * 返回view文件路径
	...
	public function set_sql_path($path){
		if($dir=$this->check_dir($path)){
			...
			throw new Exception('请设置好sql生成文件的路径!');
		}
	}
	/**
	 * 返回生成SQL文件路径 
	...
	public function set_enumerate_path($path){
		if($dir=$this->check_dir($path)){
			...
			throw new Exception('请设置好enumerate生成文件的路径!');
		}
	}
	/**
	 * 返回生成enumerate 文件路径 
	...
	public function set_code_path($path){
		if($dir=$this->check_dir($path)){
			...
			throw new Exception('请设置好code生成文件的路径!');
		}
	}
	/**
	 * 设置 生成xml文件路径
	...
	public function set_xml_path($path){
		if($dir=$this->check_dir($path)){
			...
			throw new Exception('请设置好xml生成文件的路径!');
		}
	}
	/**
	 * 设置 生成doc文件路径
	...
	public function set_doc_path($path){
		if($dir=$this->check_dir($path)){
			...
			throw new Exception('请设置好doc生成文件的路径!');
		}
	}
	/**
	 * 返回 生成 code文件路径 
	...
	public function start_mapping(){
		...
		//
		//重新生成包含全部sql的文件
		/*$file_name=$this->sql_path.'/all_data.sql';
	...
	public  function create_model($table,$table_comment=null){
		if($this->model_path){
			//生成的文件名
			$file_name=$this->model_path.'/'.strtolower($table).'.php';
			//打开文件
			...
			//待写入的内容
			$content="<?php\r\n";
			$content.="require_once ('db_oracle.php');\r\n";
			$content.="/**\r\n *注释:$table_comment\r\n *\r\n *\r\n **/\r\n";
			$content.="class T".strtolower($table)." extends dao_oracle{\r\n";
	...
	public  function create_api_xml($table,$table_comment=null){
		if($this->xml_path){
			//生成的文件名
			$file_name=$this->xml_path.'/'.strtolower($table).'.xml';
			//打开文件
			if(($fp=fopen($file_name,'w+'))===false){
				throw new Exception("打开文件{$file_name}失败,请检查权限");
			}
			...
			while ($row = oci_fetch_array ($statement,OCI_ASSOC) ) {
				$row['COLUN_NAME']=strtolower($row['COLUN_NAME']);//将字段名转成小写
				$content.="<".$row['COLUN_NAME'].">".@$row['COMMENTS']."</".$row['COLUN_NAME'].">\r\n";
			}
	...
	public  function create_api_doc($table,$table_comment=null){
		if($this->doc_path){
			//生成的文件名
			$file_name=$this->doc_path.'/'.strtolower($table).'.doc';
			//打开文件
			if(($fp=fopen($file_name,'w+'))===false){
				throw new Exception("打开文件{$file_name}失败,请检查权限");
			}
...
doc;
	...
	private function create_enumerate($table,$table_comment=null){
		...
		if($this->enumerate_path){
			//生成的文件名
			$file_name=$this->enumerate_path.'/'.$table.'.php';
			//打开文件
			if(($fp=fopen($file_name,'w+'))===false){
				throw new Exception("打开文件{$file_name}失败,请检查权限");
			}
	...
	private function create_code($table,$table_comment=null){
		if($this->code_path){
			//生成的文件名
			$file_name=$this->code_path.'/'.$table.'.php';
			//打开文件
			if(($fp=fopen($file_name,'w+'))===false){
				throw new Exception("打开文件{$file_name}失败,请检查权限");
			}
	...
	private function create_view($table,$table_comment=null){
		if($this->view_path){
			//生成的文件名
			$file_name=$this->view_path.'/'.$table.'.php';
			//打开文件
			if(($fp=fopen($file_name,'w+'))===false){
				throw new Exception("打开文件{$file_name}失败,请检查权限");
			}
	...
	private  function create_sql($table,$table_comment=null){
		if($this->sql_path){
			//生成的文件名
			$file_name=$this->sql_path.'/'.$table.'.sql';
			//打开文件
			if(($fp=fopen($file_name,'w+'))===false){
				throw new Exception("打开文件{$file_name}失败,请检查权限");
			}
	...
	public function import(){
		...
		foreach ($rows as $k=>$row1){
			//echo $row1."<br />";
			if(strpos($row1,"insert")!==false){
				continue;
			}
			$row1=trim($row1);
			$row1=rtrim($row1,';');
			//echo $row1."<br />";
			$statement2 =oci_parse($this->link,$row1);
	 ...
	 * 用于本次导入xlh数据
	 */
	public function import_xlh(){
		$area_code_array=array("511802","511821","511822","511823","511824","511825","511826","511827");
		$counter=0;
		$s=microtime(true);
		foreach ($area_code_array as $area_code){
			for($i=1;$i<=20;$i++){
				$family_file="./sql_family_".$i."_".$area_code.".sql";
				$person_file="./sql_person_".$i."_".$area_code.".sql";
				if(file_exists($family_file)){
					$rows=file($family_file);
					//var_dump($rows);
					foreach ($rows as $k=>$row1){
						//echo $row1."<br />";
						$row1=trim($row1);
		...
		$temp_file_name=$this->sql_path."/".$value;
		}
	...
	private  function create_sql_expansion($table,$table_comment=null){
		...
		if($this->sql_path){
			...
			$file_name=$this->sql_path.'/'.strtolower($table).'_expansion.sql';
			//echo $file_name;
			//打开文件
			if(($fp=fopen($file_name,'w+'))===false){
				throw new Exception("打开文件{$file_name}失败,请检查权限");
			...
			while ($row = oci_fetch_array ($statement,OCI_ASSOC) ) {
				...
				//echo "</pre>";
				if($i==0){
					//$content.="drop table ".strtolower($table)."\r\n";
					//$content.="drop table ".strtolower($table)."\r\n";
					//$content.="CREATE TABLE \"{$table}\" (\r\n";
					$content.="create table ".strtolower($table)."(";
					//$content.="\t\"".$row['COLUN_NAME']."\" ".$row['DATA_TYPE'].'('.$row['DATA_LENGTH'].') '.$nullable.' '.$default.",\r\n";
					//$content.=" ".strtolower($row['COLUN_NAME'])." ".strtolower($row['DATA_TYPE']).'('.strtolower($row['DATA_LENGTH']).') '.$nullable.' '.$default.",";
			...
			IN 
				...
				WHERE 
					...
					//$content.=" primary key (".strtolower($row['COLUN_NAME']).")";
					//$content.=")\r\n";
				}else{
					//$content.="alter table ".strtolower($table)." add ".strtolower($row['COLUN_NAME'])." ".strtolower($row['DATA_TYPE']).'('.$row['DATA_LENGTH'].') '.$nullable.' '.$default."\r\n";
					/*					if(strtolower($table)=='test'){
					echo 'ddd'.strtolower($row['COLUN_NAME']);
					}
					*/
					if(strtolower($row['COLUN_NAME'])!=$primary_key){
						...
						//echo $content.'<br />';
						//$content.="alter table ".strtolower($table)." add ".strtolower($row['COLUN_NAME'])." ".strtolower($row['DATA_TYPE']).'('.$row['DATA_LENGTH'].')' ."\r\n";
				}
				//$fieldArray[strtolower($row['COLUN_NAME'])]=strtolower($row['DATA_TYPE']);
				//字段备注不为空
				if(!empty($row['COMMENTS'])){
			...
			//查询索引
			//select t.*,i.index_type from user_ind_columns t,user_indexes i where t.index_name = i.index_name and t.table_name = i.table_name and t.table_name = 'student'

			//$sql="select * from user_ind_columns where table_name='".strtolower($table)."'";
			...
			while(1 and $rows=oci_fetch_array ($statement,OCI_ASSOC)){
				if(strtolower($rows['TABLE_NAME'])==strtolower($table)){
					...
					// ".strtolower($table)." is '".$table_comment."'\r\n";//表备注
				}
			...
			while(1 and $rows=oci_fetch_array ($statement,OCI_ASSOC)){
				...
				foreach ($rows as $key=>$value){
					...
					//$fieldArray[strtolower($row['COLUN_NAME'])]=strtolower($row['DATA_TYPE']);
					/*					老的时间类型处理方法
					if($fieldArray[strtolower($key)]=='date'){
					//to_date('1982-12-16 4:23:15','yyyy-mm-dd hh24:mi:ss')
					$values=$values."to_date('".trim($value)."','yyyy-mm-dd hh24:mi:ss'),";
					}else{
