...
/**
 * @author 我好笨
 * @todo 本类用于孕产妇管理第2-5次产前随访接口功能的具体实现
 */
require_once __SITEROOT."application/api/models/api_phs_iha_comm.php";
class phsmatwo extends api_phs_comm
{
	...
	public function ws_delete($token,$xml_string)
	{
		...
		if ($core->uuid)
		{
			...
			$prenatal_visit_two->whereAdd("ext_uuid='$ext_uuid'");
			$prenatal_visit_two->whereAdd("id='$core->uuid'");
			$prenatal_visit_two->find(true);
			if($prenatal_visit_two->uuid)
			{
				$uuid=$prenatal_visit_two->uuid;
                $org_id=$prenatal_visit_two->org_id;
				$prenatal_visit_two->free_statement();
				//删除
				$prenatal_visit_two=new Tprenatal_visit_two();
				$prenatal_visit_two->whereAdd("uuid='$uuid'");
				if (!$prenatal_visit_two->delete())
				{
					$prenatal_visit_two->free_statement();
					$status=3;
				}
	...
	public function ws_update($token,$xml_string)
	{
		...
		if ($xml_string)
		{
			...
			$dic['prenatal_visit_two']=array("pregnant_sort"=>"ma_comm","referral"=>"fksss");
			$dic['special']=array("medical_advice"=>"ma_medical_advice");//值中有'|'符号的
			$update_status=0;
	...
	public function ws_select_single($token,$xml_string)
	{
		...
		if ($core->uuid)
		{
			...
			{
			     $tag=false;
                 $prenatal_visit_two->whereAdd("uuid='$ext_uuid'");
			}
			$prenatal_visit_two->whereAdd("id='$core->uuid'");
			//$prenatal_visit_two->debugLevel(6);
			if (!$prenatal_visit_two->count("uuid"))
			{
				return $this->_error_message_start."<return_code>2</return_code><return_string>未能查找到关于身份证号：".$identity_number."，业务ID为".$ext_uuid."的相关第2-5次产前随访的记录</return_string>".$this->_error_message_end;
			}
			$prenatal_visit_two->find(true);
			require_once __SITEROOT.'library/data_arr/arrdata.php';
			$dic['prenatal_visit_two']=array("pregnant_sort"=>"ma_comm","referral"=>"fksss");
			$dic['special']=array("medical_advice"=>"ma_medical_advice");//值中有'|'符号的
			$xml_string="<?xml version='1.0' encoding='UTF-8'?><tables><table name='prenatal_visit_two'><row><identity_number>".$identity_number."</identity_number>";
			foreach ($dic['prenatal_visit_two'] as $m=>$n)
			{
				if (isset($prenatal_visit_two->$m) && isset($dic['prenatal_visit_two'][$m]))
				{
					$prenatal_visit_two->$m=array_search_for_other($prenatal_visit_two->$m,$$n);
				}
			}
			//单独处理|
			foreach ($dic['special'] as $m=>$n)
			{
				if (isset($prenatal_visit_two->$m) && isset($dic['special'][$m]))
				{
					if (strpos($prenatal_visit_two->$m,'|')===false)
					{
						//有|才做转换
			...
			{
				$prenatal_visit_two->org_id=$this->set_org_id($prenatal_visit_two->org_id);
			}
            if(!$tag)
            {
                //对ext_uuid重新赋值
                $prenatal_visit_two->ext_uuid=$ext_uuid;
            }
			$xml_string.=$prenatal_visit_two->toXML("",$exclude_array);
			$prenatal_visit_two->free_statement();
    ...
    /**
     * phsmatwo::ws_select_all()
     * 
     * 根据机构号，查询所有第2-5次产前随访记录
     * 
     * 2013-05-14
     * 
     * @param mixed $token
    ...
    public function ws_select_all($token,$xml_string)
    {
		...
		else 
		{
			...
			{
				//取得做过第2-5次产前随访的人
				$diabetes_core = new Tprenatal_visit_two();
				$diabetes_core->query("select count(*) as cou from prenatal_visit_two where prenatal_visit_two.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number')");//没有剔除死亡的人
				$diabetes_core->fetch();
				$number = $diabetes_core->cou;
				if($number>0)
				{
					$diabetes_core_select = new Tprenatal_visit_two();
					$diabetes_core_select->query("select * from prenatal_visit_two where prenatal_visit_two.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number')");//没有剔除死亡的人
					$response_str = '';
				...
				{
					return $this->_error_message_start."<return_code>2</return_code><return_string>机构号为".$org_id."的机构没有查询到第2-5次产前随访病人</return_string>".$this->_error_message_end;
				}
			}
			else 
			{
    ...
    /**
     * phsmatwo::ws_select_persons()
     * 
     * 根据个人身份证号码，查询所有第2-5次产前随访记录
     * 
     * 2013-05-14
     * 
     * @param mixed $token
    ...
    public function ws_select_persons($token,$xml_string)
    {
		...
		$organization->find(true);
		$org_id_number = $organization->id;
		//取得这个身份证和机构联查得到的所有第2-5次产前随访数据
		$diabetes_core = new Tprenatal_visit_two();
		$diabetes_core->query("select count(*) as cou from prenatal_visit_two where prenatal_visit_two.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number' and individual_core.identity_number='$identity_number')");
		$diabetes_core->fetch();
		...
		{   
			$response_str = '';
			$diabetes_core_select = new Tprenatal_visit_two();
			$diabetes_core_select->query("select * from prenatal_visit_two where prenatal_visit_two.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number' and individual_core.identity_number='$identity_number')");
			while ($diabetes_core_select->fetch())
			{
				$response_str.='<row>';
				$ext_uuid = $diabetes_core_select->uuid;
		...
		{
			return $this->_error_message_start."<return_code>2</return_code><return_string>身份证为".$identity_number."，机构号为".$org_id."的第2-5次产前随访数据未查询到！</return_string>".$this->_error_message_end;
		}
    }
}
?>