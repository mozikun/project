...
/**
 * @author mask
 * @todo 本类用于重性精神补充材料接口功能的具体实现
 */
require_once __SITEROOT."application/api/models/api_phs_iha_comm.php";


class phsschisupptbs extends api_phs_comm
{
	...
	public function __construct()
	{
		...
		require_once __SITEROOT."library/Models/schizophreniaer_supp_tabs.php";//重性精神补充材料
	...
	public function ws_delete($token,$xml_string)
	{
		...
		if ($core->uuid)
		{
			...
			$schizophreniaer_supp_tabs->whereAdd("ext_uuid='$ext_uuid'");
			$schizophreniaer_supp_tabs->whereAdd("id='$core->uuid'");
			$schizophreniaer_supp_tabs->find(true);
			if($schizophreniaer_supp_tabs->uuid)
			{
				$uuid=$schizophreniaer_supp_tabs->uuid;
				//删除重性精神补充材料
				$schizophreniaer_supp_tabs_inner=new Tschizophreniaer_supp_tabs();
				$schizophreniaer_supp_tabs_inner->whereAdd("uuid='$uuid'");
				$error=$schizophrenia_inner->showErrorMessage();
				if ($schizophreniaer_supp_tabs_inner->delete()){
					$return_string='删除成功！';
				}else {
				...
				$schizophreniaer_supp_tabs_inner->free_statement();
			...
			$schizophreniaer_supp_tabs->free_statement();
		}
		else
		{
	...
	public function ws_update($token,$xml_string)
	{
		...
		if ($xml_string)
		{
			...
			$dic['schizophreniaer_supp_tabs']=array("informed_consent"=>"cd_informed_consent","outpatient"=>"cd_outpatient","treatment_effect"=>"cd_treatment_effect","shut_case"=>"cd_shut_case","economic_conditions"=>"cd_economic_conditions");
			//值中有'|'符号的
			$dic['special']=array("main_symptomsed"=>"cd_present_symptoms");
			//定义添加还是修改状态标识
			$update_status=0;
			$update_num=0;//更新条数
			$insert_num=0;//插入条数
	...
	public function ws_select_single($token,$xml_string)
	{
		...
		if ($core->uuid){
			...
			{
			     $tag=false;
                 $schizophreniaer_supp_tabs->whereAdd("uuid='$ext_uuid'");
			}
			$schizophreniaer_supp_tabs->whereAdd("id='$core->uuid'");
			$error=$schizophreniaer_supp_tabs->showErrorMessage();
			if (!$schizophreniaer_supp_tabs->count("uuid"))
			{
				return $this->_error_message_start."<return_code>2</return_code><return_string>未能查找到关于身份证号：".$identity_number."，业务ID为".$ext_uuid."的相关重性精神补充材料记录</return_string>".$this->_error_message_end;//未能查找到关于身份证号：".$identity_number."，业务ID为".$ext_uuid."的相关重性精神补充材料记录
			}

			$schizophreniaer_supp_tabs->find(true);
			require_once __SITEROOT.'/library/data_arr/arrdata.php';
			$xml_string="<?xml version='1.0' encoding='UTF-8'?><tables><table name='schizophreniaer_supp_tabs'><row><identity_number>".$identity_number."</identity_number>";
			//转换代码

			foreach ($dic['schizophreniaer_supp_tabs'] as $m=>$n)
			{
				if (isset($schizophreniaer_supp_tabs->$m) && isset($dic[$k][$m]))
				{
					$schizophreniaer_supp_tabs->$m=array_search_for_other($schizophreniaer_supp_tabs->$m,$$n);
				}
			}
			//单独处理|
			foreach ($dic['special'] as $m=>$n)
			{
				if (isset($schizophreniaer_supp_tabs->$m) && isset($dic['special'][$m]))
				{
					if (strpos($schizophreniaer_supp_tabs->$m,'|')===false)
					{
						//有|才做转换
						...
						$schizophreniaer_supp_tabs->$m=implode('|',$temp_implode);
					}
				}
			}
			if (isset($schizophreniaer_supp_tabs->staff_id))
			{
				$schizophreniaer_supp_tabs->staff_id=$this->get_doctor_number($schizophreniaer_supp_tabs->staff_id);
			}
			if (isset($schizophreniaer_supp_tabs->org_id))
			{
				$schizophreniaer_supp_tabs->org_id=$this->set_org_id($schizophreniaer_supp_tabs->org_id);
			}
			if(!$tag)
            {
                //对ext_uuid重新赋值
                $schizophreniaer_supp_tabs->ext_uuid=$ext_uuid;
            }
			$xml_string.=$schizophreniaer_supp_tabs->toXML("",$exclude_array);
			$xml_string.="</row>";
    ...
    function ws_select_all($token,$xml_string)
    {
		...
		require_once __SITEROOT."library/Models/schizophreniaer_supp_tabs.php";
		...
		else 
		{
			...
			{
				//
				$diabetes_core = new Tschizophreniaer_supp_tabs();
				$diabetes_core->query("select count(*) as cou from schizophreniaer_supp_tabs where schizophreniaer_supp_tabs.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number')");//没有剔除死亡的人
				$diabetes_core->fetch();
				$number = $diabetes_core->cou;
				if($number>0)
				{
					$diabetes_core_select = new Tschizophreniaer_supp_tabs();
					$diabetes_core_select->query("select * from schizophreniaer_supp_tabs where schizophreniaer_supp_tabs.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number')");//没有剔除死亡的人
					$response_str = '';
				...
				{
					return $xmlhead."<return_code>2</return_code><return_string>机构号为".$org_id."的机构没有查询到的随访病人</return_string>".$xmlend;
				}
			}
			else 
			{
    ...
    function ws_select_persons($token,$xml_string)
    {
		...
		//取得这个身份证和机构联查得到的所有数据
		$diabetes_core = new Tschizophreniaer_supp_tabs();
		$diabetes_core->query("select count(*) as cou from schizophreniaer_supp_tabs where schizophreniaer_supp_tabs.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number' and individual_core.identity_number='$identity_number')");
		$diabetes_core->fetch();
		...
		{   
			$response_str = '';
			$diabetes_core_select = new Tschizophreniaer_supp_tabs();
			$diabetes_core_select->query("select * from schizophreniaer_supp_tabs where schizophreniaer_supp_tabs.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number' and individual_core.identity_number='$identity_number')");
			while ($diabetes_core_select->fetch())
			{
				$response_str.='<row>';
				$ext_uuid = $diabetes_core_select->uuid;
