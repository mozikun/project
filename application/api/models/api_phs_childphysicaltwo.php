...
/**
 * @author mask
 * @todo 本类用于1~2岁儿童健康检查接口功能的具体实现
 */
require_once __SITEROOT."application/api/models/api_phs_iha_comm.php";
class phschildphysicaltwo extends api_phs_comm
{
	...
	public function __construct()
	{
		...
		require_once __SITEROOT."library/Models/child_physical_two.php";//1~2岁儿童健康检查
	...
	public function ws_delete($token,$xml_string)
	{
		...
		if ($xml_string){
			...
			$project_array=array('5'=>'12月龄','6'=>'18月龄','7'=>'24月龄','8'=>'30月龄');;
			$project=empty($project)?5:$project;
		...
		if ($core->uuid){
			...
			$child_physical_two->whereAdd("ext_uuid='$ext_uuid'");
			$child_physical_two->whereAdd("id='$core->uuid'");
			//月龄
			$child_physical_two->whereAdd("project='$project'");
			$child_physical_two->find(true);
			if($child_physical_two->uuid){
				$uuid=$child_physical_two->uuid;
				//删除1~2岁儿童健康检查
				$child_physical_two_inner=new Tchild_physical_two();
				$child_physical_two_inner->whereAdd("uuid='$uuid'");
				$error=$child_physical_two_inner->showErrorMessage();
				if ($child_physical_two_inner->delete())
				{
				...
				$child_physical_two_inner->free_statement();
		...
		if ($xml_string){
			...
			foreach ($data_xml as $tables){
				...
				foreach ($tables as $rows){
					...
					$project=$rows->project?intval($rows->project):5;
	...
	public function ws_select_single($token,$xml_string)
	{
		...
		if ($xml_string)
		{
			...
			//反查个人uuid
			$identity_number=$where_xml->identity_number;
			//月龄array('5'=>'12月龄','6'=>'18月龄','7'=>'24月龄','8'=>'30月龄');
			$project=$where_xml->project;
			$project=empty($project)?5:$project;
			$ext_uuid=$where_xml->ext_uuid;
			if ($ext_uuid=="")
		...
		if ($core->uuid){
			...
			{
			     $tag=false;
                 $child_physical_two->whereAdd("uuid='$ext_uuid'");
			}
			$child_physical_two->whereAdd("id='$core->uuid'");
			//月龄
			//$child_physical_two->whereAdd("project='$project'");
			if (!$child_physical_two->count("uuid"))
			{
				return $this->_error_message_start."<return_code>2</return_code><return_string>未能查找到关于身份证号：".$identity_number."，业务ID为".$ext_uuid."的相关1~2岁儿童健康检查记录</return_string>".$this->_error_message_end;//未能查找到关于身份证号：".$identity_number."，业务ID为".$ext_uuid."的相关1~2岁儿童健康检查记录
			}
			$error=$child_physical_two->showErrorMessage();
			$child_physical_two->find(true);
			require_once __SITEROOT.'/library/data_arr/arrdata.php';
			$xml_string="<?xml version='1.0' encoding='UTF-8'?><tables><table name='child_physical_two'><row><identity_number>".$identity_number."</identity_number>";
			//转换代码

			foreach ($dic['child_physical_two'] as $m=>$n)
			{
				if (isset($child_physical_two->$m) && isset($dic[$k][$m]))
				{
					$child_physical_two->$m=array_search_for_other($child_physical_two->$m,$$n);
				}
			}
			//单独处理|
			foreach ($dic['special'] as $m=>$n)
			{
				if (isset($child_physical_two->$m) && isset($dic['special'][$m]))
				{
					if (strpos($child_physical_two->$m,'|')===false)
					{
						//有|才做转换
						...
						$child_physical_two->$m=implode('|',$temp_implode);
					}
				}
			}
			if (isset($child_physical_two->staff_id))
			{
				$child_physical_two->staff_id=$this->get_doctor_number($child_physical_two->staff_id);
			}
			if (isset($child_physical_two->org_id))
			{
				$child_physical_two->org_id=$this->set_org_id($child_physical_two->org_id);
			}
			if(!$tag)
            {
                //对ext_uuid重新赋值
                $child_physical_two->ext_uuid=$ext_uuid;
            }
			$xml_string.=$child_physical_two->toXML("",$exclude_array);
			$xml_string.="</row>";
    ...
    function ws_select_all($token,$xml_string)
    {
		...
		else 
		{
			...
			{
				//
				$diabetes_core = new Tchild_physical_two();
				$diabetes_core->query("select count(*) as cou from child_physical_two where child_physical_two.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number')");//没有剔除死亡的人
				$diabetes_core->fetch();
				$number = $diabetes_core->cou;
				if($number>0)
				{
					$diabetes_core_select = new Tchild_physical_two();
					$diabetes_core_select->query("select * from child_physical_two where child_physical_two.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number')");//没有剔除死亡的人
					$response_str = '';
    ...
    function ws_select_persons($token,$xml_string)
    {
		...
		$diabetes_core = new Tchild_physical_two();
		$diabetes_core->query("select count(*) as cou from child_physical_two where child_physical_two.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number' and individual_core.identity_number='$identity_number')");
		$diabetes_core->fetch();
		...
		{   
			$response_str = '';
			$diabetes_core_select = new Tchild_physical_two();
			$diabetes_core_select->query("select * from child_physical_two where child_physical_two.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number' and individual_core.identity_number='$identity_number')");
			while ($diabetes_core_select->fetch())
			{
				$response_str.='<row>';
				$ext_uuid = $diabetes_core_select->uuid;
