...
class premaritalindex extends api_phs_comm
{
	...
	public function __construct()
	{
		...
		require_once __SITEROOT."library/Models/certificate_premartial_exam.php";//婚前医学证明 
	...
	public function ws_delete($token,$xml_string)
	{
		...
		if ($core->uuid)
		{
			...
			$certificate_premartial_exam->whereAdd("ext_uuid='$ext_uuid'");
			$certificate_premartial_exam->whereAdd("id='$core->uuid'");
			$certificate_premartial_exam->find(true);
			if($certificate_premartial_exam->uuid)
			{
				$uuid=$certificate_premartial_exam->uuid;
				//删除婚前医学证明
				$certificate_premartial_exam_inner=new Tcertificate_premartial_exam();
				$certificate_premartial_exam_inner->whereAdd("uuid='$uuid'");
				$error=$certificate_premartial_exam_inner->showErrorMessage();
				if ($certificate_premartial_exam_inner->delete())
				{
			...
			$certificate_premartial_exam->free_statement();
		}
		else
		{
	...
	public function ws_update($token,$xml_string)
	{
		...
		if ($xml_string)
		{
			...
			$dic['certificate_premartial_exam']=array("blood_kinship"=>"pre_blood_kinship","medical_opinion"=>"pre_medical_opinion");
			//值中有'|'符号的
			$dic['special']=array();
			//定义添加还是修改状态标识
			$update_status=0;
	...
	public function ws_select_single($token,$xml_string)
	{
		...
		if ($core->uuid){
			...
			{
			     $tag=false;
                 $certificate_premartial_exam->whereAdd("uuid='$ext_uuid'");
			}
			$certificate_premartial_exam->whereAdd("id='$core->uuid'");
			$error=$certificate_premartial_exam->showErrorMessage();
			if (!$certificate_premartial_exam->count("uuid"))
			{
				return $this->_error_message_start."<return_code>2</return_code><return_string>未能查找到关于身份证号：".$identity_number."，业务ID为".$ext_uuid."的相关婚前医学证明记录</return_string>".$this->_error_message_end;//未能查找到关于身份证号：".$identity_number."，业务ID为".$ext_uuid."的相关婚前医学证明记录
			}
			
			$certificate_premartial_exam->find(true);
			require_once __SITEROOT.'/library/data_arr/arrdata.php';
			$xml_string="<?xml version='1.0' encoding='UTF-8'?><tables><table name='certificate_premartial_exam'><row><identity_number>".$identity_number."</identity_number>";
			//转换代码

			foreach ($dic['certificate_premartial_exam'] as $m=>$n)
			{
				if (isset($certificate_premartial_exam->$m) && isset($dic[$k][$m]))
				{
					$certificate_premartial_exam->$m=array_search_for_other($certificate_premartial_exam->$m,$$n);
				}
			}
			//单独处理|
			foreach ($dic['special'] as $m=>$n)
			{
				if (isset($certificate_premartial_exam->$m) && isset($dic['special'][$m]))
				{
					if (strpos($certificate_premartial_exam->$m,'|')===false)
					{
						//有|才做转换
						...
						$certificate_premartial_exam->$m=implode('|',$temp_implode);
					}
				}
			}
			if (isset($certificate_premartial_exam->staff_id))
			{
				$certificate_premartial_exam->staff_id=$this->get_doctor_number($certificate_premartial_exam->staff_id);
			}
			if (isset($certificate_premartial_exam->org_id))
			{
				$certificate_premartial_exam->org_id=$this->set_org_id($certificate_premartial_exam->org_id);
			}
			if(!$tag)
            {
                //对ext_uuid重新赋值
                $certificate_premartial_exam->ext_uuid=$ext_uuid;
            }
			$xml_string.=$certificate_premartial_exam->toXML("",$exclude_array);
			$xml_string.="</row>";
  ...
  /**
    ...
    function ws_select_all($token,$xml_string)
    {
		...
		else 
		{
			...
			{
				//
				$diabetes_core = new Tcertificate_premartial_exam();
				$diabetes_core->query("select count(*) as cou from certificate_premartial_exam where certificate_premartial_exam.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number')");//没有剔除死亡的人
				$diabetes_core->fetch();
				$number = $diabetes_core->cou;
				if($number>0)
				{
					$diabetes_core_select = new Tcertificate_premartial_exam();
					$diabetes_core_select->query("select * from certificate_premartial_exam where certificate_premartial_exam.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number')");//没有剔除死亡的人
					$response_str = '';
    ...
    function ws_select_persons($token,$xml_string)
    {
		...
		$diabetes_core = new Tcertificate_premartial_exam();
		$diabetes_core->query("select count(*) as cou from certificate_premartial_exam where certificate_premartial_exam.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number' and individual_core.identity_number='$identity_number')");
		$diabetes_core->fetch();
		...
		{   
			$response_str = '';
			$diabetes_core_select = new Tcertificate_premartial_exam();
			$diabetes_core_select->query("select * from certificate_premartial_exam where certificate_premartial_exam.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number' and individual_core.identity_number='$identity_number')");
			while ($diabetes_core_select->fetch())
			{
				$response_str.='<row>';
				$ext_uuid = $diabetes_core_select->uuid;
