...
class phsmapost extends api_phs_comm
{
	...
	public function ws_delete($token,$xml_string)
	{
		...
		if ($core->uuid)
		{
			...
			$postpartum_visit->whereAdd("ext_uuid='$ext_uuid'");
			$postpartum_visit->whereAdd("id='$core->uuid'");
			$postpartum_visit->find(true);
			if($postpartum_visit->uuid)
			{
				$uuid=$postpartum_visit->uuid;
				$postpartum_visit->free_statement();
				//删除
				$postpartum_visit=new Tpostpartum_visit();
				$postpartum_visit->whereAdd("uuid='$uuid'");
				if (!$postpartum_visit->delete())
				{
					$postpartum_visit->free_statement();
					$status=3;
				}
                $logs_array=array("ext_uuid"=>$ext_uuid,"org_id"=>$core->org_id,"model_id"=>7,"upload_time"=>time(),"upload_token"=>3);
	...
	public function ws_update($token,$xml_string)
	{
		...
		if ($xml_string)
		{
			...
			$dic['postpartum_visit']=array("brest"=>"ma_comm","lochia"=>"ma_comm","uterus"=>"ma_comm","durative_ulcer"=>"ma_comm","pregnant_sort"=>"ma_comm","referral"=>"fksss");
			$dic['special']=array("medical_advice"=>"po_medical_advice");//值中有'|'符号的
			$update_status=0;
	...
	public function ws_select_single($token,$xml_string)
	{
		...
		if ($core->uuid)
		{
			...
			{
			     $tag=false;
                 $postpartum_visit->whereAdd("uuid='$ext_uuid'");
			}
			$postpartum_visit->whereAdd("id='$core->uuid'");
			//$postpartum_visit->debugLevel(6);
			if (!$postpartum_visit->count("uuid"))
			{
				return $this->_error_message_start."<return_code>2</return_code><return_string>未能查找到关于身份证号：".$identity_number."，业务ID为".$ext_uuid."的相关产后随访的记录</return_string>".$this->_error_message_end;
			}
			$postpartum_visit->find(true);
			require_once __SITEROOT.'library/data_arr/arrdata.php';
			$dic['postpartum_visit']=array("brest"=>"ma_comm","lochia"=>"ma_comm","uterus"=>"ma_comm","durative_ulcer"=>"ma_comm","pregnant_sort"=>"ma_comm","referral"=>"fksss");
			$dic['special']=array("medical_advice"=>"po_medical_advice");//值中有'|'符号的
			$xml_string="<?xml version='1.0' encoding='UTF-8'?><tables><table name='postpartum_visit'><row><identity_number>".$identity_number."</identity_number>";
			foreach ($dic['postpartum_visit'] as $m=>$n)
			{
				if (isset($postpartum_visit->$m) && isset($dic['postpartum_visit'][$m]))
				{
					$postpartum_visit->$m=array_search_for_other($postpartum_visit->$m,$$n);
				}
			}
			//单独处理|
			...
			{
				$postpartum_visit->staff_id=$this->get_doctor_number($postpartum_visit->staff_id);
			}
            //2013-08-19增加处理
            if (isset($postpartum_visit->follow_staff))
			{
			     $postpartum_visit->follow_staff=$this->get_doctor_number($postpartum_visit->follow_staff);
			}
			if (isset($postpartum_visit->org_id))
			{
				$postpartum_visit->org_id=$this->set_org_id($postpartum_visit->org_id);
			}
            if(!$tag)
            {
                //对ext_uuid重新赋值
                $postpartum_visit->ext_uuid=$ext_uuid;
            }
			$xml_string.=$postpartum_visit->toXML("",$exclude_array);
			$postpartum_visit->free_statement();
    ...
    public function ws_select_all($token,$xml_string)
    {
		...
		else 
		{
			...
			{
				//取得做过产后随访的人
				$diabetes_core = new Tpostpartum_visit();
				$diabetes_core->query("select count(*) as cou from postpartum_visit where postpartum_visit.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number')");//没有剔除死亡的人
				$diabetes_core->fetch();
				$number = $diabetes_core->cou;
				if($number>0)
				{
					$diabetes_core_select = new Tpostpartum_visit();
					$diabetes_core_select->query("select * from postpartum_visit where postpartum_visit.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number')");//没有剔除死亡的人
					$response_str = '';
				...
				{
					return $this->_error_message_start."<return_code>2</return_code><return_string>机构号为".$org_id."的机构没有查询到产后随访病人</return_string>".$this->_error_message_end;
				}
			}
			else 
			{
    ...
    /**
     * phsmapost::ws_select_persons()
     * 
     * 根据个人身份证号码，查询所有产后随访记录
     * 
     * 2013-05-14
     * 
     * @param mixed $token
    ...
    public function ws_select_persons($token,$xml_string)
    {
		...
		$organization->find(true);
		$org_id_number = $organization->id;
		//取得这个身份证和机构联查得到的所有产后产前随访数据
		$diabetes_core = new Tpostpartum_visit();
		$diabetes_core->query("select count(*) as cou from postpartum_visit where postpartum_visit.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number' and individual_core.identity_number='$identity_number')");
		$diabetes_core->fetch();
		...
		{   
			$response_str = '';
			$diabetes_core_select = new Tpostpartum_visit();
			$diabetes_core_select->query("select * from postpartum_visit where postpartum_visit.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number' and individual_core.identity_number='$identity_number')");
			while ($diabetes_core_select->fetch())
			{
				$response_str.='<row>';
				$ext_uuid = $diabetes_core_select->uuid;
		...
		{
			return $this->_error_message_start."<return_code>2</return_code><return_string>身份证为".$identity_number."，机构号为".$org_id."的产后随访数据未查询到！</return_string>".$this->_error_message_end;
		}
    }
}
?>