...
/**
 * @author mask
 * @todo 本类用于3~6岁儿童健康检查接口功能的具体实现
 */
require_once __SITEROOT."application/api/models/api_phs_iha_comm.php";
class phschildphysicalthree extends api_phs_comm
{
	...
	public function __construct()
	{
		...
		require_once __SITEROOT."library/Models/child_physical_age_three.php";//3~6岁儿童健康检查
	...
	public function ws_delete($token,$xml_string)
	{
		...
		if ($xml_string){
			...
			//年龄 array('9'=>'3岁','10'=>'4岁','11'=>'5岁','12'=>'6岁')
			$age=$where_xml->age;

			$age_array	= array('9'=>'3岁','10'=>'4岁','11'=>'5岁','12'=>'6岁');
			$age=empty($age)?9:$age;
			...
			//年龄序号合法性
			//return $age;
			if (!array_key_exists(intval($age),$age_array))	{
				return $this->_error_message_start."<return_code>2</return_code><return_string>错误，请提供正确的年龄age</return_string>".$this->_error_message_end;
			}
			//判断身份证号是否合法
		...
		if ($core->uuid){
			...
			$child_physical_age_three->whereAdd("ext_uuid='$ext_uuid'");
			$child_physical_age_three->whereAdd("id='$core->uuid'");
			//年龄
			//$child_physical_age_three->debugLevel(3);
			$child_physical_age_three->whereAdd("age='$age'");
			$error=$child_physical_age_three->showErrorMessage();
			$child_physical_age_three->find(true);

			if($child_physical_age_three->uuid){
				$uuid=$child_physical_age_three->uuid;
				//删除3~6岁儿童健康检查
				$child_physical_age_three_inner=new Tchild_physical_age_three();
				$child_physical_age_three_inner->whereAdd("uuid='$uuid'");
				if ($child_physical_age_three_inner->delete())
				{
				...
				$child_physical_age_three_inner->free_statement();
			...
			$child_physical_age_three->free_statement();
		}else{
	...
	public function ws_update($token,$xml_string)
	{
		...
		if ($xml_string){
			...
			$dic['child_physical_age_three']=array("developmental_assessment"=>"bbt_developmental_assessment","complexion"=>"bbt_complexion","gait"=>"bbt_gait","eye"=>"bb_exception","ear"=>"bb_exception","heart_lung"=>"bb_exception","hepatosplenography"=>"bb_exception","behavior"=>"bbt_behavior","social"=>"bbt_behavior","hearing"=>"bbt_behavior","allergic_history"=>"bb_referral_features","referral_features"=>"bb_referral_features","abdomen"=>"bb_exception");
			//值中有'|'符号的
			...
			foreach ($data_xml as $tables){
				...
				foreach ($tables as $rows){
					...
					$age=$rows->age?intval($rows->age):9;
	...
	public function ws_select_single($token,$xml_string)
	{
		...
		if ($core->uuid){
			...
			{
			     $tag=false;
                 $child_physical_age_three->whereAdd("uuid='$ext_uuid'");
			}
			
			$child_physical_age_three->whereAdd("id='$core->uuid'");
			//年龄
			//$child_physical_age_three->whereAdd("age='$age'");
			if (!$child_physical_age_three->count("uuid"))
			{
				return $this->_error_message_start."<return_code>2</return_code><return_string>未能查找到关于身份证号：".$identity_number."，业务ID为".$ext_uuid."的相关3~6岁儿童健康检查记录</return_string>".$this->_error_message_end;//未能查找到关于身份证号：".$identity_number."，业务ID为".$ext_uuid."的相关3~6岁儿童健康检查记录
			}
			$error=$child_physical_age_three->showErrorMessage();
			$child_physical_age_three->find(true);
			require_once __SITEROOT.'/library/data_arr/arrdata.php';
			$xml_string="<?xml version='1.0' encoding='UTF-8'?><tables><table name='child_physical_age_three'><row><identity_number>".$identity_number."</identity_number>";
			//转换代码

			foreach ($dic['child_physical_age_three'] as $m=>$n)
			{
				if (isset($child_physical_age_three->$m) && isset($dic[$k][$m]))
				{
					$child_physical_age_three->$m=array_search_for_other($child_physical_age_three->$m,$$n);
				}
			}
			//单独处理|
			foreach ($dic['special'] as $m=>$n)
			{
				if (isset($child_physical_age_three->$m) && isset($dic['special'][$m]))
				{
					if (strpos($child_physical_age_three->$m,'|')===false)
					{
						//有|才做转换
						...
						$child_physical_age_three->$m=implode('|',$temp_implode);
					}
				}
			}
			if (isset($child_physical_age_three->staff_id))
			{
				$child_physical_age_three->staff_id=$this->get_doctor_number($child_physical_age_three->staff_id);
			}
			if (isset($child_physical_age_three->org_id))
			{
				$child_physical_age_three->org_id=$this->set_org_id($child_physical_age_three->org_id);
			}
			if(!$tag)
            {
                //对ext_uuid重新赋值
                $child_physical_age_three->ext_uuid=$ext_uuid;
            }
			$xml_string.=$child_physical_age_three->toXML("",$exclude_array);
			$xml_string.="</row>";
  ...
  /**
    ...
    function ws_select_all($token,$xml_string)
    {
		...
		else 
		{
			...
			{
				//
				$diabetes_core = new Tchild_physical_age_three();
				$diabetes_core->query("select count(*) as cou from child_physical_age_three where child_physical_age_three.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number')");//没有剔除死亡的人
				$diabetes_core->fetch();
				$number = $diabetes_core->cou;
				if($number>0)
				{
					$diabetes_core_select = new Tchild_physical_age_three();
					$diabetes_core_select->query("select * from child_physical_age_three where child_physical_age_three.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number')");//没有剔除死亡的人
					$response_str = '';
    ...
    function ws_select_persons($token,$xml_string)
    {
		...
		$diabetes_core = new Tchild_physical_age_three();
		$diabetes_core->query("select count(*) as cou from child_physical_age_three where child_physical_age_three.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number' and individual_core.identity_number='$identity_number')");
		$diabetes_core->fetch();
		...
		{   
			$response_str = '';
			$diabetes_core_select = new Tchild_physical_age_three();
			$diabetes_core_select->query("select * from child_physical_age_three where child_physical_age_three.id in (select individual_core.uuid from individual_core where individual_core.org_id='$org_id_number' and individual_core.identity_number='$identity_number')");
			while ($diabetes_core_select->fetch())
			{
				$response_str.='<row>';
				$ext_uuid = $diabetes_core_select->uuid;
