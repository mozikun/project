<?php
    ...
    *
    */
  class decision_equipmentController extends  controller 
  {
  	 ...
  	 public function indexAction()
  	 {
  		...
  		if($path_number<7)
  		{
  			...
  			foreach ($path_array as $k=>$v)
  			{
  				...
  				{
  					$url.= '<a href="'.__BASEPATH.'decision/equipment/index/region_id/'.$url_path->id.'"><font color="red">'.$url_path->zh_name.'</font><a>';
  				}
  				else 
  				{
  					$url.= '<a href="'.__BASEPATH.'decision/bedpie/equipment/region_id/'.$url_path->id.'">'.$url_path->zh_name.'<a>&nbsp;&nbsp;&nbsp;&nbsp;>';
  				}
  	...
  	public function getdataAction()
  	{
	     	...
	     	for($i=$o;$i>0;$i--)//近3年内的数据
	  	    {
                ...
                $year = $end_time-$i;//获得年限
  	     		...
  	     		$organization->query("select id from organization where region_path like '$region->region_path%'");
  	     		$bed_number = 0;//每个床位数
  	     		while ($organization->fetch())
  	     		{	
                    $org_ext_3 =  new Torg_ext_3();
                    $org_ext_3->query("select equipment_total_number,five_equipment,over_100_equipment from org_ext_3 where year=$year and id=$organization->id");
                    $org_ext_3->fetch();
                    $bed_number_lone = empty($org_ext_3->$k)?0:$org_ext_3->$k;
  	   ...
  	   // print_r($data);
  	  echo xml_draw_line($data,$x_array,"台数","台数","设备信息",500,500);
  	}
  }
?>