<?php
    ...
    *
    */
  class decision_personsController extends  controller 
  {
  	 ...
  	 public function indexAction()
  	 {
  		...
  		if($path_number<7)
  		{
  			...
  			foreach ($path_array as $k=>$v)
  			{
  				...
  				{
  					$url.= '<a href="'.__BASEPATH.'decision/persons/index/region_id/'.$url_path->id.'"><font color="red">'.$url_path->zh_name.'</font><a>';
  				}
  				else 
  				{
  					$url.= '<a href="'.__BASEPATH.'decision/persons/index/region_id/'.$url_path->id.'">'.$url_path->zh_name.'<a>&nbsp;&nbsp;&nbsp;&nbsp;>';
  				}
  	...
  	public function getdataAction()
  	{
  		...
  		$o = empty($y)?3:$y;
  		//取得一个地区下的每年的所有机构的 人员数
  		$region_id = $this->_request->getParam('region_id');
  		$end_time = $this->_request->getParam('end_time');
  	    $colum_array = array('gp_team_count'=>'医技人员数','gp_count'=>'医生数','community_nurse_count'=>'护士数 ','residenter_per_gp'=>'每位医生服务人数','residenter_per_nurse'=>'每位护士服务人数');
  	    $row = array();
  	    $j = 0;
	     	...
	     	for($i=$o;$i>0;$i--)//近3年内的数据
	  	    {
                ...
                $year = $end_time-$i;//获得年限
  	     		...
  	     		{	
                    $org_ext_6 =  new Torg_ext_6();
                    $org_ext_6->query("select * from org_ext_6 where year=$year and id=$organization->id");
                    $org_ext_6->fetch();
                    $bed_number_lone = empty($org_ext_6->$k)?0:$org_ext_6->$k;
  	   ...
  	   // print_r($data);
  	  echo xml_draw_line($data,$x_array,"人数","人","机构人员信息",500,500);
  	}
  }
?>