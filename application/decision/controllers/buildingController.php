<?php
    ...
    *
    */
  class decision_buildingController extends  controller 
  {
  	 ...
  	 public function indexAction()
  	 {
  		...
  		if($path_number<7)
  		{
  			...
  			foreach ($path_array as $k=>$v)
  			{
  				...
  				{
  					$url.= '<a href="'.__BASEPATH.'decision/building/index/region_id/'.$url_path->id.'"><font color="red">'.$url_path->zh_name.'</font><a>';
  				}
  				else 
  				{
  					$url.= '<a href="'.__BASEPATH.'decision/building/index/region_id/'.$url_path->id.'">'.$url_path->zh_name.'<a>&nbsp;&nbsp;&nbsp;&nbsp;>';
  				}
  	...
  	public function getdataAction()
  	{
  		...
  		$end_time = $this->_request->getParam('end_time');
  	    $colum_array = array('area'=>'房屋建筑面积','operation_area'=>'业务用房面积','peril_area'=>'危房面积','hire_area'=>'租房面积总面积','finish_area'=>'本年房屋竣工面积');
  	    $row = array();
  	    $j = 0;
	     	...
	     	for($i=$o;$i>0;$i--)//近3年内的数据
	  	    {
                ...
                $year = $end_time-$i;//获得年限
  	     		...
  	     		{	
                    ...
                    $org_ext_2->query("select * from org_ext_2 where year=$year and id=$organization->id");
                    $org_ext_2->fetch();
                    $bed_number_lone = empty($org_ext_2->$k)?0:$org_ext_2->$k;
  	   ...
  	   // print_r($data);
  	  echo xml_draw_line($data,$x_array,"平方数","平方米","建筑信息",500,500);
  	}
  }
?>