 ...
 *
 */
class children_childrenController extends controller{
	public function init(){
		...
		require_once __SITEROOT."library/Models/child_physical.php";//1岁以内儿童健康检查记录表
	}
    /**
     * 按个人列表
     *
...
//		}
		...
		if ($search['staff_id'])
		{
			$child_physical->whereAdd("child_physical.staff_id = '".$search['staff_id']."'");
		}
		if ($search['username'])
		{
		...
		if($search['created_start_time']!=''){
			...
			$child_physical->whereAdd("child_physical.vist_time>={$created_start_time}");
		}		
		//结束时间查询
		if($search['created_end_time']!=''){
			$created_end_time=strtotime($search['created_end_time']);
			$child_physical->whereAdd("child_physical.vist_time<={$created_end_time}");
		}	
		$child_physical->whereAdd($individual_core_region_path_domain);	
		$nums=$child_physical->count("distinct child_physical.id");
		...
		$child_physical->selectAdd("child_physical.id as id,individual_core.standard_code_1,individual_core.identity_number,individual_core.name as name,individual_core.name_pinyin as name_pinyin,count(child_physical.uuid) as snums");
		$child_physical->groupBy("child_physical.id,individual_core.name,individual_core.name_pinyin,individual_core.standard_code_1,individual_core.identity_number");
...
//		}
	...
	public  function listAction(){
		...
		if ($userid)
		{
			...
			$project			= array('1'=>'满月','2'=>'3月龄','3'=>'6月龄','4'=>'8月龄');
			...
			$rickets_signs_array= $bb_rickets_signs_array;
			...
			//指导|checkbox|1=>喂养指导,2=>母乳指导,3=>疾病预防
			$advising			= $bb_advising;

			$this->view->project_options 		=$project; //项目|radio|1=>满月,2=>3月龄,3=>6月龄,4=>8月龄
			...
			$child_physical->orderby("project ASC");
			$child_physical->find();
			$child_physical_array=array();
			$i=0;
			while ($child_physical->fetch())
			{
				...
				$project_index										= $child_physical->project;//项目|radio|1=>满月,2=>3月龄,3=>6月龄,4=>8月龄
				$child_physical_array[$i]['project_current'] 		= $child_physical->project;//项目|radio|1=>满月,2=>3月龄,3=>6月龄,4=>8月龄
                ...
                $complexion_index									= array_search_for_other($child_physical->complexion,$complexion);//面色国家代码
				$child_physical_array[$i]['complexion_current']  	= $complexion_index==""?"":$complexion[$complexion_index][1];//面色|radio|1=>红润,2=>其它

				$skin_index											= array_search_for_other($child_physical->skin,$skin);//国家代码 
				$child_physical_array[$i]['skin_current'] 		 	= $skin_index==""?"":$skin[$skin_index][1];//皮肤|radio|1=>未见异常,2=>异常
				
				$bregma_index										= array_search_for_other($child_physical->bregma,$bregma);//
				$child_physical_array[$i]['bregma_current']		 	= $bregma_index==""?"":$bregma[$bregma_index][1];//前囟|radio|1=>闭合,2=>未闭
				$child_physical_array[$i]['bregma_width'] 		 	= $child_physical->bregma_width;//前囟宽
				$child_physical_array[$i]['bregma_height'] 		 	= $child_physical->bregma_height;//前囟高

				$cervical_mass_index								= array_search_for_other($child_physical->cervical_mass,$cervical_mass);//
				$child_physical_array[$i]['cervical_mass']		 	= $cervical_mass_index==""?"":$cervical_mass[$cervical_mass_index][1];//颈部包块 	1有  2 无
				
				$eye_index											= array_search_for_other($child_physical->eye,$eye);
				$child_physical_array[$i]['eye_current'] 		 	= $eye_index==""?"":$eye[$eye_index][1];//眼|radio|1=>未见异常,2=>异常

				$ear_index											= array_search_for_other($child_physical->ear,$ear);
				$child_physical_array[$i]['ear_current'] 		 	= $ear_index==''?'':$ear[$ear_index][1];//耳|radio|1=>未见异常,2=>异常
				
				$oralcavity_index									= array_search_for_other($child_physical->oralcavity,$oralcavity);
				$child_physical_array[$i]['oralcavity'] 		 	= $oralcavity_index==''?'':$ear[$oralcavity_index][1];//口腔|radio|1=>未见异常,2=>异常
				...
				$umbilical_region_index								 =array_search_for_other( $child_physical->umbilical_region,$umbilical_region);
				...
				$limb_index											= array_search_for_other($child_physical->limb,$limb);
				$child_physical_array[$i]['limb_current'] 		 	= $limb_index==''?'':$limb[$limb_index][1];//四肢|radio|1=>未见异常,2=>异常

				$gait_index											= array_search_for_other($child_physical->gait,$gait);
				$child_physical_array[$i]['gait_current'] 			= $gait_index==''?'':$gait[$gait_index][1];//步态|radio|1=>未见异常,2=>异常

				$rickets_symptom_index								= array_search_for_other($child_physical->rickets_symptom,$rickets_symptom);
				...
				$rickets_signs_index								= array_search_for_other($child_physical->rickets_signs,$rickets_signs);
				$child_physical_array[$i]['rickets_signs_current'] 	= $rickets_signs_index==''?'':$rickets_signs[$rickets_signs_index][1];//佝偻病体征|

				$gmzz_index											= array_search_for_other($child_physical->gmzz,$gmzz);
				...
				$child_physical_array[$i]['field_sport'] 			= $child_physical->field_sport;//户外活动小时/日
				$child_physical_array[$i]['vitamin_d'] 				= $child_physical->vitamin_d;//服用维生素D
				...
				$between_and_vist_time_index						= array_search_for_other($child_physical->between_and_vist_time,$between_and_vist_time);
		...
		{
			$url=array("1岁以内儿童健康检查记录列表"=>__BASEPATH.'children/children/index',"用户列表"=>__BASEPATH.'iha/index/index');
	...
	public function addAction(){
		...
		$project_num		=   $this->_request->getParam('project','1');//月龄
		$child_physical_chk	=	new Tchild_physical();
		...
		if(empty($uuid)){
			...
			//修改
			$child_physical 					=	new Tchild_physical();
			$child_physical->whereAdd("uuid='$uuid'");
			$child_physical->find();
			$child_physical->fetch();

			$this->view->uuid 					= $child_physical->uuid;//编号
			$this->view->staff_id 				= $child_physical->staff_id;//医生档案号
			$this->view->id 					= $child_physical->id;//个人档案号
			$this->view->created 				= $child_physical->created;//添加记录时间
			$individual_inf=get_individual_info($child_physical->id);//取得个人信息中所有信息
			if($this->haveWritePrivilege){
				$this->view->name		 		= $individual_inf->name;//居民姓名
				$this->view->serial_num 		= $individual_inf->standard_code_1;//标准档案号
			}else{
			...
			//$this->view->project_options 		=$project; //项目|radio|1=>满月,2=>3月龄,3=>6月龄,4=>8月龄,5=>12月龄,6=>18月龄,7=>24月龄,8=>30月龄,
			$this->view->project_current 		= $child_physical->project;//项目|radio|1=>满月,2=>3月龄,3=>6月龄,4=>8月龄,5=>12月龄,6=>18月龄,7=>24月龄,8=>30月龄,
			$this->view->vist_time 				= empty($child_physical->vist_time)?'':adodb_date("Y-m-d",$child_physical->vist_time);//随访日期
			$this->view->weight 				= $child_physical->weight;//体重(kg)

			$this->view->weight_info_options 	= $weight_info; //体重情况|radio|1=>上,2=>中,3=>下
			$this->view->weight_info_current 	= $child_physical->weight_info;//体重情况|radio|1=>上,2=>中,3=>下
			$this->view->height 			 	= $child_physical->height;//身长(cm)

			$this->view->height_info_options 	= $height_info; //身长情况|radio|1=>上,2=>中,3=>下
			$this->view->height_info_current 	= $child_physical->height_info;//身长情况|radio|1=>上,2=>中,3=>下
			$this->view->head_size 			 	= $child_physical->head_size;//头围（cm）

			//$this->view->complexion_options   =  $complexion; //面色|radio|1=>红润,2=>其它
			$this->view->complexion_current  	= array_search_for_other($child_physical->complexion,$complexion);//面色|radio|1=>红润,2=>其它

			$this->view->skin_options 			= $skin; //皮肤|radio|1=>未见异常,2=>异常
			$this->view->skin_current 		 	= array_search_for_other($child_physical->skin,$skin);//皮肤|radio|1=>未见异常,2=>异常

			//$this->view->bregma_options 		=$bregma; //前囟|radio|1=>闭合,2=>未闭
			$this->view->bregma_current		 	= array_search_for_other($child_physical->bregma,$bregma);//前囟|radio|1=>闭合,2=>未闭
			$this->view->bregma_width 		 	= $child_physical->bregma_width;//前囟宽
			$this->view->bregma_height 		 	= $child_physical->bregma_height;//前囟高
			
			$this->view->cervical_mass_current 	= array_search_for_other($child_physical->cervical_mass,$cervical_mass);//颈部包块|radio|1=>有,2=>无

			//$this->view->eye_options			=$eye; //眼|radio|1=>未见异常,2=>异常
			$this->view->eye_current 		 	= array_search_for_other($child_physical->eye,$eye);//眼|radio|1=>未见异常,2=>异常

			//$this->view->ear_options =$ear; //耳|radio|1=>未见异常,2=>异常
			$this->view->ear_current 		 	= array_search_for_other($child_physical->ear,$ear);//耳|radio|1=>未见异常,2=>异常
			
			$this->view->hearing_current 		= array_search_for_other($child_physical->hearing,$hearing);//听力|radio|1=>通过,2=>未通过
			
			$this->view->oralcavity_current 	= array_search_for_other($child_physical->oralcavity,$oralcavity);//口腔|radio|1=>未见异常,2=>异常
			
			$this->view->number_of_teeth 	 	= $child_physical->number_of_teeth;//出牙数(颗)

			//$this->view->heart_lung_options 	=$heart_lung; //心肺|radio|1=>未见异常,2=>异常
			$this->view->heart_lung_current  	= array_search_for_other($child_physical->heart_lung,$heart_lung);//心肺|radio|1=>未见异常,2=>异常

			//$this->view->abdomen_options 		=$abdomen; //腹部|radio|1=>未见异常,2=>异常
			$this->view->abdomen_current 	 	= array_search_for_other($child_physical->abdomen,$abdomen);//腹部|radio|1=>未见异常,2=>异常

			//$this->view->umbilical_region_options =$umbilical_region; //脐部|radio|1=>未见异常,2=>异常
			$this->view->umbilical_region_current =array_search_for_other( $child_physical->umbilical_region,$umbilical_region);//脐部|radio|1=>未见异常,2=>异常

			//$this->view->limb_options =$limb; //四肢|radio|1=>未见异常,2=>异常
			$this->view->limb_current 		 	= array_search_for_other($child_physical->limb,$limb);//四肢|radio|1=>未见异常,2=>异常

			//$this->view->gait_options 		=$gait; //步态|radio|1=>未见异常,2=>异常
			$this->view->gait_current 			= array_search_for_other($child_physical->gait,$gait);//步态|radio|1=>未见异常,2=>异常

			//$this->view->rickets_symptom_options =$rickets_symptom; //佝偻病症状|radio|1=>无,2=>夜惊,3=>多汗,4=>烦躁
			$this->view->rickets_symptom_current = array_search_for_other($child_physical->rickets_symptom,$rickets_symptom);//佝偻病症状|radio|1=>无,2=>夜惊,3=>多汗,4=>烦躁

			//$this->view->rickets_signs_options =$rickets_signs; //佝偻病体征|radio|1=>无,2=>颅骨软化,3=>方颅,4=>枕秃,5=>肋串珠,6=>肋外翻,7=>肋软骨沟,8=>鸡胸,9=>手镯征,10=>O型腿,11=>X型腿
			$this->view->rickets_signs_current 	= array_search_for_other($child_physical->rickets_signs,$rickets_signs);//佝偻病体征|radio|1=>无,2=>颅骨软化,3=>方颅,4=>枕秃,5=>肋串珠,6=>肋外翻,7=>肋软骨沟,8=>鸡胸,9=>手镯征,10=>O型腿,11=>X型腿

			//$this->view->gmzz_options =$gmzz; //肛门/外生殖器|radio|1=>未见异常,2=>异常
			$this->view->gmzz_current 			= array_search_for_other($child_physical->gmzz,$gmzz);//肛门/外生殖器|radio|1=>未见异常,2=>异常
			$this->view->hemoglobin 			= $child_physical->hemoglobin;//血红蛋白值
			$this->view->field_sport 			= $child_physical->field_sport;//户外活动小时/日
			$this->view->vitamin_d 				= $child_physical->vitamin_d;//服用维生素D

			//$this->view->developmental_assessment_options =$developmental_assessment; //发育评估|radio|1=>通过,2=>未过
			$this->view->developmental_assessment_current = array_search_for_other($child_physical->developmental_assessment,$developmental_assessment);//发育评估|radio|1=>通过,2=>未过

			//$this->view->between_and_vist_time_options =$between_and_vist_time; //两次随访间患病情况|radio|1=>未患病,2=>患病
			$this->view->between_and_vist_time_current = array_search_for_other($child_physical->between_and_vist_time,$between_and_vist_time);//两次随访间患病情况|radio|1=>未患病,2=>患病
			$this->view->other 					= $child_physical->other;//其它

			//$this->view->referral_features_options =$referral_features; //转诊|radio|1=>无,2=>有
			$this->view->referral_features_current = array_search_for_other($child_physical->referral_features,$referral_features);//转诊|radio|1=>无,2=>有
			$this->view->reason 				= $child_physical->reason;//转诊原因
			$this->view->agencies_departments 	= $child_physical->agencies_departments;//转诊机构及科室
			...
			$this->view->advising_other 		= $child_physical->advising_other;//指导其它
			$this->view->next_followup_time 	= empty($child_physical->next_followup_time)?'':adodb_date("Y-m-d",$child_physical->next_followup_time);//下次随访日期

			$region_users						= empty($this->user['current_region_path_domain'])?'':region_users($this->user['current_region_path_domain']);//所有的医生
			//print_r($region_users);
			$this->view->region_users			= $region_users;
			$this->view->doctors_signature  	= $child_physical->doctors_signature;//随访医生签名
	...
	public function  updateAction(){
		...
		$child_physical->project 					= $project_number;//项目|radio|1=>满月,2=>3月龄,3=>6月龄,4=>8月龄,5=>12月龄,6=>18月龄,7=>24月龄,8=>30月龄,
		$vist_time 									= $this->_request->getParam('vist_time');//随访日期
		$child_physical->vist_time 					= empty($vist_time)?'':mkunixstamp($vist_time);//随访日期

		$child_physical->weight 					= $this->_request->getParam('weight');//体重(kg)

		$child_physical->weight_info 				= $this->_request->getParam('weight_info');//体重情况|radio|1=>上,2=>中,3=>下

		$child_physical->height 					= $this->_request->getParam('height');//身长(cm)

		$child_physical->height_info 				= $this->_request->getParam('height_info');//身长情况|radio|1=>上,2=>中,3=>下
		
		$child_physical->head_size 					= $this->_request->getParam('head_size');//头围（cm）

		$child_physical->complexion 				= array_code_change($this->_request->getParam('complexion'),$bb_complexion);//面色|radio|1=>红润,2=>其它

		$child_physical->skin 						= array_code_change($this->_request->getParam('skin'),$bb_exception);//皮肤|radio|1=>未见异常,2=>异常

		$child_physical->bregma 					= array_code_change($this->_request->getParam('bregma'),$bb_bregma);//前囟|radio|1=>闭合,2=>未闭

		$child_physical->bregma_width 				= $this->_request->getParam('bregma_width');//前囟宽

		$child_physical->bregma_height 				= $this->_request->getParam('bregma_height');//前囟高
		$child_physical->cervical_mass				= array_code_change($this->_request->getParam('cervical_mass'),$bb_cervical_mass);//颈部包块|radio|1=>有,2=>无
		$child_physical->eye 						= array_code_change($this->_request->getParam('eye'),$bb_exception);//眼|radio|1=>未见异常,2=>异常

		$child_physical->ear 						= array_code_change($this->_request->getParam('ear'),$bb_exception);//耳|radio|1=>未见异常,2=>异常
		$child_physical->hearing				 	= array_code_change($this->_request->getParam('hearing'),$bb_developmental_assessment);//听力|radio|1=>通过,2=>未过
		$child_physical->oralcavity 				= array_code_change($this->_request->getParam('oralcavity'),$bb_exception);//口腔|radio|1=>未见异常,2=>异常
		
		$child_physical->number_of_teeth 			= $this->_request->getParam('number_of_teeth');//出牙数(颗)

		$child_physical->heart_lung 				= array_code_change($this->_request->getParam('heart_lung'),$bb_exception);//心肺|radio|1=>未见异常,2=>异常

		$child_physical->abdomen 					= array_code_change($this->_request->getParam('abdomen'),$bb_exception);//腹部|radio|1=>未见异常,2=>异常

		$child_physical->umbilical_region 			= array_code_change($this->_request->getParam('umbilical_region'),$bb_exception);//脐部|radio|1=>未见异常,2=>异常

		$child_physical->limb 						= array_code_change($this->_request->getParam('limb'),$bb_exception);//四肢|radio|1=>未见异常,2=>异常

		$child_physical->gait 						= array_code_change($this->_request->getParam('gait'),$bb_exception);//步态|radio|1=>未见异常,2=>异常

		$child_physical->rickets_symptom 			= array_code_change($this->_request->getParam('rickets_symptom'),$bb_rickets_symptom);//佝偻病症状|radio|1=>无,2=>夜惊,3=>多汗,4=>烦躁

		$child_physical->rickets_signs 				= array_code_change($this->_request->getParam('rickets_signs'),$bb_rickets_signs);//佝偻病体征|radio|1=>无,2=>颅骨软化,3=>方颅,4=>枕秃,5=>肋串珠,6=>肋外翻,7=>肋软骨沟,8=>鸡胸,9=>手镯征,10=>O型腿,11=>X型腿

		$child_physical->gmzz 						= array_code_change($this->_request->getParam('gmzz'),$bb_exception);//肛门/外生殖器|radio|1=>未见异常,2=>异常

		$child_physical->hemoglobin 				= $this->_request->getParam('hemoglobin');//血红蛋白值

		$child_physical->field_sport 				= $this->_request->getParam('field_sport');//户外活动小时/日

		$child_physical->vitamin_d 					= $this->_request->getParam('vitamin_d');//服用维生素D

		$child_physical->developmental_assessment 	= array_code_change($this->_request->getParam('developmental_assessment'),$bb_developmental_assessment);//发育评估|radio|1=>通过,2=>未过

		$child_physical->between_and_vist_time 		= array_code_change( $this->_request->getParam('between_and_vist_time'),$bb_between_and_vist_time);//两次随访间患病情况|radio|1=>未患病,2=>患病

		$child_physical->other 						= $this->_request->getParam('other');//其它

		$child_physical->referral_features 			= array_code_change($this->_request->getParam('referral_features'),$bb_referral_features);//转诊|radio|1=>无,2=>有

		$child_physical->reason 					= $this->_request->getParam('reason');//转诊原因

		$child_physical->agencies_departments 		= $this->_request->getParam('agencies_departments');//转诊机构及科室
		...
		$child_physical->advising 					= $advising_str;//指导|checkbox|1=>喂养指导,2=>母乳指导,3=>疾病预防

		$child_physical->advising_other 			= $this->_request->getParam('advising_other');//指导其它
		$next_followup_time 						= $this->_request->getParam('next_followup_time');//下次随访日期
		$child_physical->next_followup_time 		= empty($next_followup_time)?'':mkunixstamp($next_followup_time);//下次随访日期*/

		$child_physical->doctors_signature 			= $this->_request->getParam('doctors_signature');//随访医生签名
		if(empty($uuid)){
			//添加
			$uuid						=uniqid("cd",true);//编号

			$child_physical->uuid = $uuid;//编号

			$child_physical->staff_id 	= $this->user['uuid'];//医生档案号
			$individual_session 		= new Zend_Session_Namespace("individual_core");

			$child_physical->id 		= empty($serial_number)?$individual_session->uuid:$serial_number;//个人档案号

			$child_physical->created 	= time();//添加记录时间
			//$child_physical->debugLevel(8);
			$update_token				= $child_physical->insert();

		}else{
			//修改
			$child_physical->whereAdd("uuid='$uuid'");
			$update_token				= $child_physical->update();

		}
		if($update_token==true){
			message("更新成功！",array("1岁以内儿童健康检查录表列表"=>__BASEPATH.'children/children/index',"添加"=>__BASEPATH."children/children/add","修改"=>__BASEPATH."children/children/add/id/$serial_number/project/$project_number"));
		}else{
			message("更新失败！",array("1岁以内儿童健康检查录表列表"=>__BASEPATH.'children/children/index',"添加"=>__BASEPATH."children/children/add","修改"=>__BASEPATH."children/children/add/id/$serial_number/project/$project_number"));
		}
	}
	/**
	 * 删除操作
	...
	public function delAction(){
		...
		$child_physical	=new Tchild_physical();
		$child_physical->whereAdd("uuid='$uuid'");
		if($child_physical->delete()){
			message("删除成功！",array("1岁以内儿童健康检查录表列表"=>__BASEPATH.'children/children/index',"添加"=>__BASEPATH."children/children/add"));
		}else {
			message("删除失败！",array("1岁以内儿童健康检查录表列表"=>__BASEPATH.'children/children/index',"添加"=>__BASEPATH."children/children/add"));
		}
	}

	/**
	 * 
	 * 获取child_physical最新的一条记录取得访问时间和随访医生
	 *
