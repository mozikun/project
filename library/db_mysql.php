...
class dao_mysql implements db_abstract{
	...
	public function select($invoke=true){
		...
		*/
		...
		//这里把字段命名冲突的事交给使用者本身了，如果自定义字段与已有字段冲突，系统后报错
		...
		//$invoke默认为true，为falsh时，为一些只需要生成where语句的功能服务。
		if($invoke){
			...
			//需要缓存处理的就是这句
			$this->_dao_rs=mysql_query($query_string);
		}
		return true;
	}
	//用于兼容pear
	...
	public function fetch(){
		...
		if(!empty($this->_dao_count)){
			...
			if(!($this->_dao_cache_token and $this->_dao_cache_available)){
				$row=mysql_fetch_array($this->_dao_rs,MYSQL_ASSOC);
				$this->_dao_cache_rows[0]=$row;
				//加记录结束标志
				$this->_dao_cache_rows[1]=false;
				//使用缓存时才进行,并且对于count的缓存单独进行,具没有缓存过。
				if($this->_dao_cache_token){
		...
		if(empty($this->_dao_field_list) or (!empty($this->_dao_field_list) and $this->_dao_field_list_add==true)){
			...
			foreach ($var as $key=>$value){
				...
				if(substr($key,0,1)!='_'){
					...
					$this->$key=isset($row[$field])?$row[$field]:"";
					//注入到_fetch_rest_rows
					$this->_dao_fetch_rest_rows[]=array($this,$alias,strtolower($key));
					$this->_dao_to_array[$key]=isset($row[$field])?$row[$field]:"";
				}
	...
	public function free_statement(){
		@mysql_free_result($this->_dao_rs);
		//return ;
	}
...
class db_connect{
	...
	public static function getInstance($host){
		if(self::$instance === null){
			self::$instance = new db_connect($host);
		}
