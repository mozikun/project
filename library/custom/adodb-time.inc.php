...
/**
 * 将指定格式日期转换为unix时间戳
 * @param $time 格式如2010-09-21的日期
 * @param $parmer 分隔符
 */
function mkunixstamp($time,$parmer='')
{
	$temp=array();
	if ($parmer=='')
	{
	...
	foreach ($parmer as $k=>$v)
	{
		...
		if (strlen($temp[2])>2)
		{
			...
			{
				$sec=explode(":",$tt[1]);
				return adodb_mktime(intval($sec[0]),intval($sec[1]),intval($sec[2]),$temp[1],$tt[0],$temp[0]);
			}
		}
		else 
		{
			return adodb_gmmktime(0,0,0,$temp[1],$temp[2],$temp[0]);
		}
	}
}
function check_utime($time,$parmer=array('-')){
	if($time == ''){
		//$time = date('Y-m-d',time());
		return null;
	}
	return mkunixstamp($time,$parmer);
}
?>