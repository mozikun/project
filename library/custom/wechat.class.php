 ...
 * 
 * 我好笨 对此类进行扩展，未经同意，请勿使用于除雅安项目以外的第三方项目
 *
 */
class Wechat {
	/* 配置参数  */
...
// 			return $result['ErrCode'];
	...
	public function getContactInfo($fakeid)
	{
		if (file_exists($this->cookiefilepath)||true===$this->login())
		{
			$url = $this->protocol."://mp.weixin.qq.com/cgi-bin/getcontactinfo?t=ajax-getcontactinfo&lang=zh_CN&fakeid=".$fakeid."&token=".$this->webtoken;
    ...
    public function getFakeId()
    {
        if (file_exists($this->cookiefilepath)||true===$this->login())
		{
			$url = $this->protocol."://mp.weixin.qq.com/cgi-bin/contactmanagepage?t=wxm-friend&lang=zh_CN&pagesize=100&pageidx=0&type=0&groupid=0&token=".$this->webtoken;
			$response = $this->get($url, $this->protocol."://mp.weixin.qq.com/");
            preg_match_all('|"fakeId" : "(.*)",
      "nickName": "(.*)",|Uis',$response,$tmp);
			if($tmp[1] && $tmp[2])
            {
                $result['fakeid']=$tmp[1];
                $result['name']=$tmp[2];
    ...
    public function getTopMsg()
    {
        if (file_exists($this->cookiefilepath)||true===$this->login())
		{
			$url = $this->protocol."://mp.weixin.qq.com/cgi-bin/getmessage?t=ajax-message&lang=zh_CN&count=1&timeline=0&day=0&star=0&cgi=getmessage&offset=0";
            $submit = array('ajax'=>1,'token'=>$this->webtoken);
			$response = $this->post($url,$submit, $this->protocol."mp.weixin.qq.com/cgi-bin/getmessage?t=wxm-message&lang=zh_CN&count=50&token=".$this->webtoken);
            $result = json_decode($response,1);
			if($result[0]['fakeId'])
            {
				return $result[0];
