 ...
 * 普通变量替换成$this->_data['xxx']的形式 内部变量根据实际情况进行处理
 * 生成编译后的文件
 * 
 *
 */
class view{
	...
	private function processVariable($label,$line_number){
	...
	private function covert($label){
		...
		if(substr($label,0,1)=='$'){
			...
			foreach ($var_array[1] as $var){
				...
				if($normal_var){
					//这里与前面对变量的处理方法基本一致，但由于分解得到的全是变量名，因此不需要分别按简单变更与数组变量进行处理了
					$code='$this->_data[\''.$var.'\']';
					//$this->_file_array[$line_number]=str_replace($if_condition_var,$code,$this->_file_array[$line_number]);
					$label=str_replace('$'.$var,$code,$label);
					//echo $label.__LINE__.'----------------<br />';
				}
			}
			$code='<?php echo '.$label.';?>';
			$this->_file_array[$line_number]=str_replace($this->left_delimiter.$old_label.$this->right_delimiter,$code,$this->_file_array[$line_number]);
		...
		if(substr($label,0,7)=='foreach'){
			...
			//$foreach_array=array_unique($foreach_array);
			...
			if($this->_foreach_counter-1==0){
				//已是顶层，则变量值肯定来源于主程序，所以其值来自于$this->_data
				$var_name=substr($foreach_var_array[1],1,strlen($foreach_var_array[1])-1);
				$foreach_var_array[1]='$this->_data[\''.$var_name.'\']';
			}
			if($this->_foreach_counter-1>0){
		...
		if(substr($label,0,2)=='if'){
			...
			preg_match_all('~\$(\w*)~i',$label,$var_array);
			/*			echo "<pre>";
			var_dump($if_condition_var_array);
			echo "</pre>";*/
			foreach ($var_array[1] as $var){
				$normal_var=true;
				//判断是否是foreach循环中的变量
